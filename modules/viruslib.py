...
def run_merge_contigs(**context):
    ...
    logger = create_simple_logger("viruslib")
...
def run_vclust_dedup(**context):
    ...
    # 从配置文件获取默认值
    config = get_config()
    min_ident = params_dict.get(
        "--min-ident",
        config["parameters"]["vclust_params"].split("--min-ident ")[1].split()[0],
    )
    out_ani = params_dict.get(
        "--out-ani",
        config["parameters"]["vclust_params"].split("--out-ani ")[1].split()[0],
    )
    out_qcov = params_dict.get(
        "--out-qcov",
        config["parameters"]["vclust_params"].split("--out-qcov ")[1].split()[0],
    )
    ani = params_dict.get(
        "--ani", config["parameters"]["vclust_params"].split("--ani ")[1].split()[0]
    )
    qcov = params_dict.get(
        "--qcov", config["parameters"]["vclust_params"].split("--qcov ")[1].split()[0]
    )
...
def run_phabox2_prediction(**context):
    ...
    # 获取数据库路径
    if config.has_section("database") and config["database"].get("phabox2_db"):
        dbdir = config["database"]["phabox2_db"]
    else:
...
def run_taxonomy_annotation(**context):
    ...
    if config.has_section("database") and config["database"].get("genomad_db"):
        db_dir = config["database"]["genomad_db"]
    else:
...
def run_eggnog(**context):
    ...
    config = context["config"]
    logger = create_simple_logger("viruslib")
    if config.has_section("database") and config["database"].get("eggnog5"):
        db_dir = config["database"]["eggnog5"]
    else:
