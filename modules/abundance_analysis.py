...
from utils.common import create_simple_logger, prepare_directory, run_command
...
def run_coverm_contig(**context):
    ...
    if not prepare_directory(coverm_dir, logger):
        return False
    ...
    ret = run_command(cmd, logger, "coverm_contig")
    if ret != 0:
        logger.error("coverm contig analysis failed")
        return False
...
def run_coverm_gene(**context):
    ...
    ret = run_command(cmd, logger, "coverm_gene")
    if ret != 0:
        logger.error("coverm gene analysis failed")
        return False
...
def run_abundance_analysis(**context):
    ...
    # 运行contig丰度分析
    if not run_coverm_contig(**context):
        logger.error("Contig abundance analysis failed")
        return False
    
    # 运行基因丰度分析
    if not run_coverm_gene(**context):
        logger.error("Gene abundance analysis failed")
        return False
