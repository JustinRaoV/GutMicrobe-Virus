...
class PerformanceMonitor:
    ...
    def _get_current_usage(self) -> ResourceUsage:
        ...
        try:
            ...
            if current_net and self.initial_net:
                network_sent_mb = (
                    current_net.bytes_sent - self.initial_net.bytes_sent
                ) / (1024 * 1024)
                network_recv_mb = (
                    current_net.bytes_recv - self.initial_net.bytes_recv
                ) / (1024 * 1024)
            else:
                network_sent_mb = 0
                network_recv_mb = 0
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_disk_write_mb": sum(
                u.disk_io_write_mb for u in self.resource_history
            ),
            "total_network_sent_mb": sum(
                u.network_sent_mb for u in self.resource_history
            ),
            "total_network_recv_mb": sum(
                u.network_recv_mb for u in self.resource_history
            ),
        }
    ...
    def get_optimization_suggestions(self) -> List[str]:
        ...
        memory_high_threshold = float(
            config["parameters"]["monitor_memory_high_threshold"]
        )
...
def create_monitor(
    log_file: Optional[str] = None, interval: float = 5.0
) -> PerformanceMonitor:
    """创建性能监控器"""
