...
class PipelineLogger:
    ...
    def step_complete(self, step_name: str, step_number: int):
        ...
        elapsed = time.time() - self.start_time
        estimated_total = (
            elapsed * self.total_steps / step_number if step_number > 0 else 0
        )
    ...
    def _update_progress(
        self, step_number: int, step_name: str, status: str, error: str = ""
    ):
        """更新进度文件"""
...
def create_logger(
    output_dir: str, sample: str, log_level: str = "INFO"
) -> Tuple[PipelineLogger, ErrorHandler]:
    """创建日志管理器和错误处理器"""
