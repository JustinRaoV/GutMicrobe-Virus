...
def parameter_input():
    parser = argparse.ArgumentParser(description='GutMicrobe Virus Downstream Analysis Pipeline')
    parser.add_argument('input1', help='Path to the fastq(.gz) file of read1 (for sample name extraction)')
    parser.add_argument('input2', help='Path to the fastq(.gz) file of read2 (for sample name extraction)')
    parser.add_argument('--upstream-result', help='Path to upstream analysis result directory', required=True)
    parser.add_argument('--viruslib-result', help='Path to viruslib pipeline result directory', required=True)
    parser.add_argument('-o', '--output', help='Output directory', default=os.path.join(os.getcwd(), 'abundance_result'))
...
def build_context(args):
    ...
    if len(fastq_files) != 2:
        raise ValueError(f"Expected 2 fastq files in {host_removed_dir}, found {len(fastq_files)}")
    ...
    viruslib_contig_path = os.path.join(args.viruslib_result, "2.vclust_dedup", "viruslib_nr.fa")
    ...
    viruslib_gene_path = os.path.join(args.viruslib_result, "5.cdhit_dedup", "gene_cdhit.fq")
...
def main():
    ...
    logger, error_handler = setup_logging(
        context['output'], context['sample'], args.log_level
    )
    ...
    try:
        logger.step_start("abundance_analysis", 1, 1)
        ...
        logger.step_failed("abundance_analysis", 1, str(e))
        raise
