...
def run_spades(output, threads, sample1, sample2):
    ...
    print(
        f"spades.py -1 {output}/3.bowtie2/{sample1}.fastq -2 {output}/3.bowtie2/{sample2}.fastq -t {threads} -o {output}/5.spades")
    ret = subprocess.call([
        f"spades.py -1 {output}/3.bowtie2/{sample1}.fastq -2 {output}/3.bowtie2/{sample2}.fastq -t {threads} -o {output}/5.spades"],
        shell=True)
    if ret != 0:
...
def run_virsorter(output, threads):
    ...
    ret = subprocess.call(
        [f"virsorter run -w {output}/7.vircontigs -i {output}/6.filter/contig_1k.fasta -j {threads} all"], shell=True)
    if ret != 0:
