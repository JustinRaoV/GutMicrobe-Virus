...
def parameter_input():
    ...
    parser.add_argument('--host',
                        help='Name(s) of Bowtie2 index(es) built before, please separate different names with "," when removing more than one hosts (for example: "human,mouse") (default: None, which means this step will be skipped)',
                        default=None)
    parser.add_argument('-a', '--adapter', type=int,
                        help='Choose the adapter file used by trimmomatic to remove adapter sequences (please input the interger code, 0:NexteraPE-PE, 1:TruSeq3-PE-2, 2:TruSeq3-PE, 3:TruSeq2-PE, default:NexteraPE-PE)',
                        default=0)
    ...
    parser.add_argument('-r', '--remove_inter_result', action='store_true',
                        help='Remove intermediate results (use carefully, this will only keep the final viral contigs, fastqc and viromeqc reports!)',
                        default=False)
...
if __name__ == '__main__':
    ...
    if log < 2:
        if args.adapter == 1:
            adapter = f"{db}/adapters/TruSeq3-PE-2.fa"
        elif args.adapter == 2:
            adapter = f"{db}/adapters/TruSeq3-PE.fa"
        elif args.adapter == 3:
